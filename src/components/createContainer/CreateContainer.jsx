...
const CreateContainer = () => {
  ...
  const uploadImage = (e) => {
    ...
    const storageRef = ref(storage, `img/${Date.now()}-${imageFile.name}`);
  ...
  const saveDetails = () => {
    setIsLoading(true);
    try {
      if (!title || !province || !imageAsset || !price || !location) {
        setFields(true);
    ...
    <div className="w-full min-h-screen flex items-center justify-center">
      <div className="w-[90%] md:w-[50%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4">
        ...
        <div className="w-full py-2 border-b border-gray-300 flex items-center gap-2">
          <IoMdHome className="text-xl text-gray-700" />
          <input
            type="text"
            required
            value={title}
        ...
        <div
          className="group flex justify-center items-center flex-col border-2 border-dotted border-gray-300 w-full h-225 
        md:h-340 cursor-pointer rounded-lg"
        >
          {isLoading ? (
            <Loader />
          ) : (
        ...
        <div className="w-full flex flex-col md:flex-row items-center gap-3">
          <div className="w-full py-2 border-b border-gray-300 flex items-center gap-2">
            <ImLocation2 className="text-gray-700 text-2xl" />
            <input
              type="text"
              required
              value={location}
